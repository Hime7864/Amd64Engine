	...
	case 0xC0:
	{
		...
		if (ptr)
		{
			...
			while (tempCount)
			{
				...
				*(UINT8*)ptr = (*(UINT8*)ptr * 2) + FLAGS.CF;
				FLAGS.CF = tempCF;
				tempCount--;
			}
	...
	case 0xC1:
	{
		...
		if (ptr)
		{
			if (Prefix.W)
			{
				...
				while (tempCount)
				{
					auto tempCF = get_msb(*(UINT64*)ptr, 64);
					*(UINT64*)ptr = (*(UINT64*)ptr * 2) + FLAGS.CF;
					FLAGS.CF = tempCF;
					tempCount--;
				}
			...
			else if (!Prefix.OperandSize)
			{
				...
				while (tempCount)
				{
					auto tempCF = get_msb(*(UINT32*)ptr, 32);
					*(UINT32*)ptr = (*(UINT32*)ptr * 2) + FLAGS.CF;
					FLAGS.CF = tempCF;
					tempCount--;
				}
			...
			else
			{
				...
				while (tempCount)
				{
					auto tempCF = get_msb(*(UINT16*)ptr, 16);
					*(UINT16*)ptr = (*(UINT16*)ptr * 2) + FLAGS.CF;
					FLAGS.CF = tempCF;
					tempCount--;
				}
